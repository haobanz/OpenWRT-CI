...
local ifc = uci:get("accelerator", "base", "neigh")
if ifc == nil then
  ifc = "br-lan"
end
...
if fs.access("/proc/net/arp") then
  ...
  for line in io.lines("/proc/net/arp") do
    ...
    if dev == ifc and flag == "0x2" then
      -- get current name
      local name = mac
      if dhcp_map[key] ~= nil and dhcp_map[key].name ~= "*" then
...
for key, item in pairs(arp_map) do
  ...
  elseif typ >= 20 and typ <= 22 then
    catalog = "vr_catalog"
  else
    catalog = "unknown_catalog"
  end
  -- device type
  local device_typ = device:taboption(catalog, ListValue, key, item.name)
...
-- Apply device classifications without requiring a separate service action.
m.apply_on_parse = true
m.on_after_apply = function()
  util.exec("/etc/init.d/acc restart >/dev/null 2>&1 &")
end
