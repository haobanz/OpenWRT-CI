...
- 预装实验性的 `biubiu-acc` 与 LuCI 管理页（登录、内置游戏选择和预检已实现）
...
保存到 root 私有文件，通过原子替换完成续期，并仅输出脱敏状态。0.8.0 的
`服务 -> biubiu 加速器` 管理页可完成短信登录、会话续期、整个 LAN 或指定设备
作用域选择、Steam/CS2/Epic 内置游戏选择、公钥导入、自检、日志查看和只读
实时匹配观测；原始游戏/区服/平台 ID 仅保留在高级设置中。内置目录记录官方
公开的身份域名、
内容/CDN 排除项与高置信端口提示，不会采用覆盖大部分端口的粗放规则。管理
进程默认关闭。0.8.0 已加入游戏目录/搜索、权益检查、节点 profile、数据通道
授权与续期 API，生成 root 私有 runtime，并实现 Bolt v3 TCP/UDP 数据通道。
启动加速时按整个 LAN 或指定设备安装 nftables TPROXY 规则；默认采用独占策略，
会先停止 OpenClash，停止加速后恢复。只有账号、公钥、节点、授权和 runtime
全部就绪时才允许接管，失败会自动清理策略路由与规则。Bolt 扩展参数仍需在
授权账号上完成一次真实服务端握手验证，页面会如实显示失败状态。
