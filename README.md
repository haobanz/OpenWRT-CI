...
- 预装实验性的 `biubiu-acc` 与 LuCI 管理页（登录、官方游戏/区服选择、LAN 匹配和预检）
...
使用另一套独立 key/IV 的 ADAT 信封，支持厂商公开 RSA 材料及密钥轮换，不包含
私钥或捕获的会话。OpenWrt C 客户端会将稳定设备 ID 和登录会话保存到 root
私有文件，通过原子替换完成续期，并仅输出脱敏状态。
`服务 -> biubiu 加速器` 管理页可完成短信登录、会话续期、整个 LAN 或指定设备
作用域选择、Steam/CS2/Epic 内置游戏选择、公钥导入、自检、日志查看和只读
实时匹配观测。加速配置可搜索官方游戏，再选择该游戏的区服与加速模式；游戏、
区服和模式会经后端校验后用于实际控制请求，原始 ID 仅保留在高级设置中。
内置 Steam/CS2/Epic 多选是 LAN 流量匹配规则，不等同于官方加速游戏选择。
内置目录记录官方
公开的身份域名、
内容/CDN 排除项与高置信端口提示，不会采用覆盖大部分端口的粗放规则。管理
进程默认关闭。已加入游戏目录/搜索、权益检查、节点 profile、数据通道
授权与续期 API，生成 root 私有 runtime，并实现经典 Bolt TCP/UDP 数据通道。
启动加速时按整个 LAN 或指定设备安装 nftables TPROXY 规则；默认采用独占策略，
会先停止正在运行的 OpenClash、daed 或 dae，停止加速后只恢复本轮暂停的服务，
原先关闭的代理保持关闭。只有账号、公钥、节点、授权和 runtime
全部就绪时才允许接管，失败会自动清理策略路由与规则。本地开发机已完成真实
信令登录、续期、14 条 TCP/UDP 通道绑定及 TCP/UDP 收发验证。
2026-09-06 在 NN6000 v2 上安装并热修验证了实际 TPROXY TCP/UDP 收发、
Steam 连接规则命中与 OpenClash 停止/恢复，转发进程 RSS 约 4.2 MB。
daed/dae 未运行状态保持关闭及合成启停回归已通过，实际运行中的 daed/dae
切换和 CS2 对局延迟、丢包仍待验收。当前节点由服务端自动分配，手动节点与入口选择尚未
接入，不能把底层收发成功等同于完整客户端交互或实战加速已完成。
